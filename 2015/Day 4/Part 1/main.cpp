...
int main(int argc, char *argv[])
{
    ...
    //Creating an output variable to find the output that gives use 5 leading zeros.
    std::string output{md5(hex + std::to_string(number))};

    //Running a while loop that increases the number variable by 1 until the output has 5 leading zeros.
    while (output.substr(0,5) != "00000")
    {
        number++;
